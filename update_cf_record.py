...
def generate_handler(p: argparse.Namespace) -> function[str]:
    '''
    ...
    def delete_record():
        ...
        extra_params["name"] = p.name
        extra_params["type"] = p.type

        if p.content:
            extra_params["content"] = p.content
        if p.ttl:
            extra_params["ttl"] = p.ttl
        if p.proxied:
            extra_params["proxied"] = p.proxied
        if p.priority:
            extra_params["priority"] = p.priority

        return delete_dns_record(p.cf, p.zone_id, p.name, p.type, p.force, extra_params = extra_params)
    ...
    def set_record():
        ...
        return dns_update(p.cf, p.zone_id, p.name, p.content, p.type, p.force, extra_params = extra_params)

    def ddns_handler():
        print('MY IP: {} {}'.format(p.content, p.type))
        return set_record()
    ...
    def get_zones():
        zones = get_zones(p.cf)
        for zone in zones:
            print(zone['name'])

        return True

    def get_handler(name: str) -> function:
        func = set_record

        if name == "ddns":
            func = ddns_handler
        elif name == "get-zone-id":
            func = get_zone_id
        elif name == "get-zones":
            func = get_zones
        elif name == "delete":
