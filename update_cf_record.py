...
def delete_dns_record(cf: CF.CloudFlare, zone_id: str, params: dict) -> bool:
...
def generate_handler(p: argparse.Namespace) -> function[str]:
    ...
    if p.ttl:
        params["ttl"] = p.ttl
    if p.proxied:
        params["proxied"] = p.proxied
    if p.priority:
        params["priority"] = p.priority

    def delete_record():
        return delete_dns_record(p.cf, p.zone_id, params)
...
def main():
    ...
    # run command handler
    ret = generate_handler(parsed)(parsed.command)()
    if (ret):
        exit(EXIT_SUCCESS)
